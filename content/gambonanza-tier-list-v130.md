---
category: "Strategy & Guides"
title: "Gambonanza Tier List v1.3.0 - All Gambits Ranked After the Balance Patch"
description: "Updated Gambonanza tier list for v1.3.0. Templar drops from S to B, Missigno and Graal join the pool, 6 Gambits move up. All 150+ Gambits ranked S/A/B/C with the latest balance data."
game_version: ">=v1.3.0"
last_reviewed: "2026-07-13"
review_status: "current"
date: "2026-07-13"
---

{{< callout type="verdict" title="v1.3.0 Tier List at a Glance" >}}
This tier list replaces the v1.1.0 version. **Templar drops from S to B-tier** (nerfed from 1/1 to 1/3). Six Gambits enter S and A-tier thanks to guaranteed triggers. Missigno's debuts in S-tier for lategame pivot potential. If you're still playing the v1.1.0 meta, your tier assumptions are wrong.
{{< /callout >}}

{{< diagram src="tier-comparison-v11-v13.svg" alt="Tier comparison between v1.1.0 and v1.3.0" caption="Green arrows = moved up. Red arrows = moved down. New Gambits enter at their debut tier." >}}

## S-Tier (Game-Warping)

| Gambit | Why | Best For |
|--------|-----|----------|
| **Missigno's Gambit** NEW | Sell to randomize all pieces. Lategame hail-mary. | Stage 4+ pivots |
| **Catapult's Gambit** | ROOK capture launches diagonally. Board control king. | Any build |
| **Whip's Gambit** buffed to 1/1 | Guaranteed trigger now. Elite economy. | Economy builds |
| **Dead or Alive's Gambit** buffed to 1/1 | Guaranteed payout on capture. S-tier now. | Aggressive builds |
| **Graal's Gambit** NEW | Free Blessing Tile on every Gambit pickup. | Tile-based strats |
| **Bunker's Gambit** reworked | Global +$1 per PROTECT. Consistent income all run. | Protect builds |

## A-Tier (Strong)

| Gambit | Why |
|--------|-----|
| **Show Jumping** 1/1 | Guaranteed jump. Knight synergy peak. |
| **Pendant** 1/1 | Guaranteed pendant. |
| **Chamberlain** 1/1 | Consistent piece generation now. |
| **Miracle** 1/1 | Took 3 patches to get here, but finally reliable. |
| **Princess's Gambit** | Bless ADJACENT piece. ADJACENT keyword clarified. |
| **Excalibur's Gambit** | ADJACENT KNIGHTs +1 attack. Cluster strat. |

## B-Tier (Situational)

| Gambit | Why |
|--------|-----|
| **Templar's Gambit** nerfed to 1/3 | Former S-tier, now unreliable. Skip unless BISHOP-heavy. |
| **Family's Gambit** | PAWN wall +1 move. Solid but narrow. |
| **Scout's Gambit** | ADJACENT tile reveal. Good for exploration. |

## C-Tier (Niche)

| Gambit | Why |
|--------|-----|
| **Makibishi's Gambit** | Trap on ADJACENT. Still weak even after fix. |
| **Lich's Gambit** nerfed v1.2 | Never recovered from the v1.2 nerf. |
| **Squire's Gambit** | No Luck on KNIGHT move. Too situational. |
| **Mercenary's Gambit** | Enemy summon cost increase. Marginal. |
| **Super Hero Landing** | Landing pad tiles. Almost never optimal. |

## Removed in v1.3.0

- **Shield's Gambit** replaced by Bubble Shield's
- **Trojan's Gambit** replaced by Jockey's

### How to Adjust Your Tier Thinking

If you have a tier list from v1.1.0 taped to your wall, here's what to unlearn:

1. **Templar isn't a free pick anymore.** Stop taking it as your first Gambit. Wait until you have multiple BISHOPs.
2. **Don't sleep on Show Jumping.** At 1/1 guaranteed, it's as good as any economy Gambit now.
3. **Dead or Alive's is no longer a gamble.** Every capture pays. It's one of the best A-tier Gambits.
4. **Missigno's is a pivot tool, not a pickup.** Don't grab it early. Don't grab it unless you have 8+ pieces.
5. **Bunker's is worth buying.** At global +$1 per PROTECT, it's the most consistent income Gambit in the game.

### Tier List FAQ

**Q: Why is Miracle's Gambit only A-tier when it's 1/1 now?**
A: Its effect (transform a piece) is good but situational. The 1/1 guarantee makes it reliable, but you won't always want to transform your pieces. In contrast, Whip's and Dead or Alive's generate unconditional value.

**Q: Is Catapult still S-tier in v1.3?**
A: Yes. Catapult's board control value is unchanged by the patch. The ADJACENT keyword clarification actually helps by confirming diagonal launch works off adjacent captures.

**Q: Should I ever pick up Lich's Gambit?**
A: Only if you're running a very specific sacrifice build and have nothing better. The v1.2 nerf was significant enough that Lich is a last-resort Gambit in v1.3.

**Q: What about Graal's vs Miracle's?**
A: Graal's is better for consistent tile upgrades across a full run. Miracle's is better for a single targeted transformation. Graal's wins in most builds.

## How the Meta Shifted

| Archetype | v1.1 Tier | v1.3 Tier | Why |
|-----------|-----------|-----------|-----|
| Templar-centric | S | B | 1/1 to 1/3 nerf |
| Protect Economy | B | A | Bunker's global +$1 |
| Knight Rush | B | A | Show Jumping + Whip both 1/1 |
| Lich builds | B | C | v1.2 nerf + no recovery |
| Blessed Tile | C | A | Graal's + blessed buffs |
| High-roll pivot | C | S | Missigno's enables pivots |

## Community Resources

- [Official v1.3.0 Patch Notes (Steam)](https://store.steampowered.com/news/app/3509230/view/689761349249532405)
- [Full 200+ Gambit Database (Wiki)](https://gambonanza.fandom.com/wiki/Gambits)

---

*Tier list updated for Gambonanza v1.3.0 (June 25, 2026). Supersedes the v1.1.0 tier list. Check our [v1.3.0 Patch Breakdown](/v130-patch-breakdown/) for a complete analysis of all changes.*

